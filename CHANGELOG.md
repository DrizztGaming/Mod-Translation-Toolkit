...
### Changed
- DLL/UI diagnostics now include a short **How to read this report** section below the raw findings.
- Added explanations for Tooltip/UI markers, RimWorld translation API markers, Harmony patch markers and method-name clues.
- Added a short interpretation note explaining that multiple UI + Harmony signals suggest code-generated/modified UI, but do not automatically prove a hardcoded string.
- Diagnostic explanation follows the selected Toolkit UI language (Polish/English).
- Updated both bundled manuals with the same report-reading guide.

## 0.4.2 - 2026-08-02

### Changed
- DLL/UI diagnostics no longer run during normal mod loading.
- Assembly scanning starts only after pressing **DLL/UI diagnostics**.
- Added visible `Skanowanie DLL/UI...` status while diagnostics are running.
- Diagnostics inspect only the root assembly folder plus the active/newest supported version.
- Common dependency/runtime DLLs such as Harmony, HugsLib, Newtonsoft.Json, System/Microsoft/Unity libraries are skipped.
- Removed the broad `GUI` keyword that produced false positives such as `Guid`.
- Added targeted markers including `DoTimeControlsGUI`, `Prefix`, `Postfix`, `Transpiler` and `HarmonyPatch`.

### Result
- Normal mod selection should be noticeably faster.

## 0.4.1 - 2026-08-02

### Added
- Added **DLL/UI diagnostics** for RimWorld mod assemblies.
- Toolkit now heuristically scans DLL string/metadata content for `Hotkey`, `KeyBindingDef`, `KeyBindingDefOf`, `Tooltip`, `TooltipHandler`, `Gizmo`, `Command`, `Widgets`, `Translate` and related UI markers.
- Added a dedicated **DLL/UI diagnostics** dialog with per-DLL evidence.
- Added `MANUAL_PL.md` and `MANUAL_EN.md` next to the launcher in the release package.
- Manuals cover RimWorld translation, existing-translation updates, search/replace, diagnostics, Workshop dashboard and Kenshi.

### Notes
- DLL diagnostics are heuristic and intentionally read-only. Toolkit does not patch assemblies.
- A detected API/string reference is evidence for investigation, not proof that the visible UI text can be overridden through language files.

## 0.4.0 - 2026-08-02

### Added
- Added **Keybind diagnostics** for RimWorld mods.
- Toolkit now explicitly scans `KeyBindingDef`.
- `label` and `description` fields exposed by KeyBindingDefs continue through the normal DefInjected translation workflow.
- KeyBindingDefs without a translatable `label` or `description` are reported as possible **UI/code-generated text**.
- Added a diagnostics dialog listing affected keybind defNames and source files.
- Scan status now reports detected KeyBindingDefs and how many require UI diagnostics.

### Why
- This helps identify cases such as Smart Speed, where visible text such as a hotkey tooltip may not exist in the mod's normal language files and may instead be generated by RimWorld UI or code.

## 0.3.9 - 2026-08-02

### Added
- Added a new **Workshop** dashboard.
- Creator profile stores a translation-author name and optional SteamID/profile reference locally.
- Saved creator name is automatically used in the RimWorld **Translation author** field.
- Workshop publications can be added by Steam Workshop URL or PublishedFileID.
- Public item details are refreshed through Steam's `GetPublishedFileDetails` Web API.
- Dashboard aggregates subscriptions, favorites and views across tracked publications.
- Added per-item title, PublishedFileID, AppID, statistics and update timestamps.
- Workshop items can be opened directly in Steam Community from the dashboard.
- Local tracked-item list persists between Toolkit launches.

### Security
- Toolkit does not ask for or store a Steam password.
- Toolkit does not embed a Steamworks publisher API key.
- Automatic enumeration of every creator Workshop item is intentionally deferred because the official query endpoint requires authenticated Web API access.
